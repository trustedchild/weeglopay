...
class CoinPaymentController extends Controller
{
    ...
    public function trx_info(Request $req)
    {
            ...
            if ($payment['error'] == 'ok' && (INT) $user->coinpayment_transactions()->where('payment_id', $req->result['txn_id'])->count('id') === 0)
            {
                ...
                elseif (isset($req->payload['type']) && $req->payload['type'] == "merchant") //merchant
                {
                    ...
                    try
                    {
                        ...
                        $response = $this->helper->sendTransactionNotificationToAdmin('payment', ['data' => $deposit]);
        ...
        else
        {
            ...
            {
                if ($payment['error'] == 'ok' && (int) CointpaymentLogTrx::where('payment_id', $req->result['txn_id'])->count('id') === 0)
                {
                    $data = $payment['result'];
                    ...
                    try
                    {
                        ...
                        CointpaymentLogTrx::create($saved);
    ...
    public function receive_webhook(Request $req)
    {
        ...
        $log = CointpaymentLogTrx::where('payment_id', $req->txn_id)->first();
        if ($log != null)
        {
            $log->update([
                'status'      => $req->status,
