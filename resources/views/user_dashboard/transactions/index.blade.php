                                            ...
                                            <tr click="0" data-toggle="collapse" data-target="#collapseRow{{$key}}"
                                                ...
                                                @else
                                                    @if($transaction->total > 0)
                                                        <td class="text-left text-success"><p>{{ $transaction->currency->type != 'fiat' ? "+".$transaction->total : "+".formatNumber($transaction->total) }}</p></td>

                                                    @elseif($transaction->total == 0)
                                                        <td class="text-left"><p>{{ formatNumber($transaction->total) }}</p></td>
                                                    @elseif($transaction->total < 0)
                                                        <td class="text-left text-danger">
                                                            <p>{{ $transaction->currency->type != 'fiat' ? $transaction->total : formatNumber($transaction->total) }}</p>
                                                        </td>
                                                    @endif
                                                @endif
                                            </tr>
