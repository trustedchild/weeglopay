...
class CryptoSendController extends Controller
{
    ...
    public function sendCryptoConfirm(Request $request)
    {
        ...
        else
        {
            try
            {
                ...
                // dd($getReceiverAddressCryptoApiLog);
    ...
    public function sendCryptoSuccess(Request $request)
    {
        ...
        $validateUserBalanceAgainstAmount = $this->validateUserBalanceAgainstAmount($request);
        // dd($validateUserBalanceAgainstAmount);
        if ($validateUserBalanceAgainstAmount->getData()->status == 400)
        {
        ...
        else
        {
            try
            {
                try
                {
                    // dd($request['walletCurrencyCode'], $cryptoTrx['senderAddress'],$cryptoTrx['receiverAddress'], $cryptoTrx['amount'], $cryptoTrx['uniqueCode']);

                    //Call withdraw API of block io
                    $withdrawInfo = $this->cryptoCurrency->withdrawOrSendAmountToReceiverAddress($request['walletCurrencyCode'], $cryptoTrx['senderAddress'],
                        $cryptoTrx['receiverAddress'], $cryptoTrx['amount'], $cryptoTrx['uniqueCode']);
                }
                catch (\Exception $e)
                {
