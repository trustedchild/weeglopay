...
class MerchantPaymentController extends Controller
{
    ...
    public function merchantPaymentCsv()
    {
        ...
        $data['merchant_payments'] = $merchant_payments = $this->merchant_payment->getMerchantPaymentsList($from, $to, $status, $currency, $pm)->orderBy('merchant_payments.id', 'desc')->get();
        // dd($deposits);
                ...
                elseif ($request->transaction_status == 'Success') //current status
                {
                    ...
                    if (isset($merchant_payment->merchant))
                    {
                        ...
                        if (!empty($merchant_payment->merchant->user->carrierCode) && !empty($merchant_payment->merchant->user->phone))
                        {
                            ...
                            $merchant_status_sms_info_msg = str_replace('{uuid}', $merchant_payment->uuid, $merchant_status_sms_info_msg);
                            $merchant_status_sms_info_msg = str_replace('{status}', $merchant_payment->status, $merchant_status_sms_info_msg);
                            ...
                            {
                                sendSMS($merchant_payment->merchant->user->carrierCode . $merchant_payment->merchant->user->phone, $merchant_status_sms_info_msg);
                            }
                        }
                    }
                ...
                elseif ($request->transaction_status == 'Success') //done
                {
                    ...
                    if (isset($merchant_payment->user_id))
                    {
                        ...
                        if (!empty($paid_by_user->user->carrierCode) && !empty($paid_by_user->user->phone))
                        {
                            ...
                            if (checkAppSmsEnvironment())
                            {
                                sendSMS($paid_by_user->user->carrierCode . $paid_by_user->user->phone, $merchant_status_sms_info_msg);
                            }
                        }
                    }
