...
class CurrenciesDataTable extends DataTable
{
    public function ajax() //don't use default dataTable() method
    {
        return datatables()
            ...
            ->addColumn('action', function ($currency)
            {
                ...
                $edit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url('admin/settings/edit_currency/' . encrypt($currency->id)) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_currency')) ? '<a href="' . url('admin/settings/delete_currency/' . encrypt($currency->id)) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;' : '';
    ...
    {
        $query = Currency::select('id','type','name','code','symbol','rate','logo','status','default')->orderBy('id', 'desc');
